...
static void destroy_stream_session(obs_pipewire_stream_data *obs_pw)
{
	obs_enter_graphics();
...
static inline bool has_effective_crop(obs_pipewire_stream_data *obs_pw)
{
...
static bool prepare_obs_frame(obs_pipewire_stream_data *obs_pw,
...
static bool build_format_params(obs_pipewire_stream_data *obs_pw,
...
static void init_format_info_media(obs_pipewire_stream_data *obs_pw)
{
...
static void remove_modifier_from_format(obs_pipewire_stream_data *obs_pw,
...
static void renegotiate_format(void *data, uint64_t expirations)
{
	...
	obs_pipewire_stream_data *obs_pw = (obs_pipewire_stream_data *)data;
	...
	if (!build_format_params(obs_pw, &pod_builder, &params, &n_params)) {
		teardown_pipewire_stream(obs_pw);
...
const struct pw_stream_events stream_events_media = {
...
obs_pipewire_stream_create(int pipewire_fd, int pipewire_node, const char *name,
			   ...
			   const struct pw_stream_events *stream_events,
			   enum obs_import_type import_type, obs_source_t *source)
{
